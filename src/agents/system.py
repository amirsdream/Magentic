...
class MetaAgentSystem:
    ...
    def process_query(
        ...
        for layer_idx, agent_indices in enumerate(execution_layers):
            logger.info(f"\n{'='*60}")
            logger.info(f"🔀 LAYER {layer_idx + 1}/{len(execution_layers)}: Executing {len(agent_indices)} agents")
            logger.info(f"{'='*60}")
            
            if len(agent_indices) > 1:
                self.visualizer.display_parallel_agents_start(
                    [plan.agents[i] for i in agent_indices], 
                    layer_idx + 1, 
            ...
            if len(agent_indices) == 1:
                output = self._execute_single_agent(
                    agent_indices[0], plan.agents[agent_indices[0]], plan.agents, 
                    outputs, query, depth, max_depth, trace, layer_idx, len(execution_layers)
                )
                outputs[agent_indices[0]] = output
            else:
                layer_outputs = asyncio.run(self._execute_layer_parallel(
                    agent_indices, plan.agents, outputs, query, depth, max_depth, trace, layer_idx, len(execution_layers)
                ))
        ...
        for i in agent_indices:
            task = asyncio.create_task(
                self._execute_agent_with_limit(
                    i, all_agents[i], all_agents, completed_outputs, query, depth, max_depth, layer_idx, total_layers
                )
            )
            tasks.append((i, task))
    ...
    async def _execute_agent_with_limit(
        ...
        async with self._semaphore:
            ...
            result = await self._execute_agent_async(
                agent_index, agent_spec, all_agents, completed_outputs, query, depth, max_depth, layer_idx, total_layers
            )
    ...
    async def _execute_agent_async(
        ...
        result = await loop.run_in_executor(
            ...
            role, task, query, previous_outputs, self.conversation_history, depth, max_depth, self.process_query
        )
    ...
    async def execute_agent_for_langgraph(
        ...
        result = await loop.run_in_executor(
            None,
            self.agent_executor.execute,
            role_obj, task, original_query, previous_outputs, conv_hist, 0, 3, self.process_query
        )
