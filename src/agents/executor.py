...
class AgentExecutor:
    ...
    def _track_tokens(self, response: Any) -> TokenUsage:
        ...
        if self._current_agent_id and self._current_role:
            return tracker.add_agent_usage(
    ...
    def execute(
        ...
        if role.needs_tools:
            ...
            return self._execute_without_tools(
                role, system_msg, task_msg, config, 
                task, depth, max_depth, process_query_callback
            )
...
"""
    ...
    def _execute_researcher(
        ...
        search_prompt = SystemMessage(content="""You are a research specialist planning a web search.
...
IMPORTANT: Keep search queries short and focused.""")
        ...
        if tool_results:
            ...
            return {
                ...
                "tool_calls": [{"name": search_tool.name, "args": {"query": q}} for q in search_queries]
            }
    ...
    def _execute_standard_tool_calling(
        ...
        logger.info(f"🔧 {role.name} bound with {len(role_tools)} tools: {[t.name for t in role_tools[:5]]}...")
        
        response = llm_with_tools.invoke([system_msg, task_msg], config=config)
        self._track_tokens(response)  # Track token usage
        
        if hasattr(response, 'tool_calls') and response.tool_calls:
            return self._process_tool_calls(role, system_msg, task_msg, response, config, role_tools)
    ...
    def _process_tool_calls(
        ...
        if tool_results:
            ...
            final_response = self.llm.invoke([
                system_msg,
                task_msg,
                AIMessage(content=f"Based on these results:\n\n{tool_context}\n\nProvide a comprehensive answer.")
            ], config=config)
            self._track_tokens(final_response)  # Track token usage
    ...
    def _execute_tool(self, tool_name: str, tool_args: dict, role_tools: List[BaseTool]) -> Optional[str]:
