...
class ExecutionVisualizer:
    ...
    def create_execution_graph(
        ...
        # Add start node
        net.add_node("start", label="START", shape="box", color="#4CAF50",
                    font={"size": 16, "color": "white"}, title=plan_description)
        
        # Add agent nodes and edges
        last_agents = self._add_agent_nodes(net, agents, trace, parallel_agents)
        
        # Add end node
        net.add_node("end", label="END", shape="box", color="#4CAF50",
                    font={"size": 16, "color": "white"}, title="Execution complete")
    ...
    def _add_agent_nodes(
        ...
        colors = {
            "researcher": "#2196F3", "analyzer": "#9C27B0", "planner": "#FF9800",
            "writer": "#E91E63", "coder": "#00BCD4", "critic": "#F44336",
            "synthesizer": "#4CAF50", "coordinator": "#FF5722"
        }
        ...
        for i, agent in enumerate(agents):
            ...
            net.add_node(node_id, label=f"{i}. {role}", shape="circle", color=color,
                        font={"size": 14, "color": "white"}, title=title, size=35 if not depends_on else 30)
            
            if not depends_on:
                net.add_edge("start", node_id, color={"color": "#4CAF50", "opacity": 0.8},
                            width=3, title="Runs immediately")
            else:
                for dep_idx in depends_on:
                    if 0 <= dep_idx < len(agents):
                        net.add_edge(f"agent_{dep_idx}", node_id, color={"color": "#888", "opacity": 0.7},
                                    width=2, title=f"Waits for Agent {dep_idx}")
    ...
    def display_summary(self, result: Dict[str, Any]) -> None:
        ...
        panel = Panel("\n".join(summary_lines), title="[bold cyan]📊 Execution Summary[/bold cyan]",
