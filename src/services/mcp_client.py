...
class MCPClient:
    ...
    async def health_check(self, retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(retries):
            try:
                ...
                logger.info(f"MCP health check successful: {result.get('healthy_servers', 0)}/{result.get('total_servers', 0)} servers")
                return result
            except httpx.ConnectError as e:
                last_error = e
                logger.warning(f"MCP health check attempt {attempt + 1}/{retries}: Connection refused to {url}")
                if attempt < retries - 1:
                    await asyncio.sleep(2)  # Wait before retry
            except httpx.TimeoutException as e:
                last_error = e
                logger.warning(f"MCP health check attempt {attempt + 1}/{retries}: Timeout connecting to {url}")
                if attempt < retries - 1:
                    await asyncio.sleep(2)
            except Exception as e:
                last_error = e
                logger.warning(f"MCP health check attempt {attempt + 1}/{retries}: {type(e).__name__}: {e}")
                if attempt < retries - 1:
                    await asyncio.sleep(2)
    ...
    async def discover_tools(self, force_refresh: bool = False) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.info(f"Discovered {data.get('total_tools', 0)} tools from {len(tools_by_server)} servers")
    ...
    async def get_tools_for_role(self, role: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Role '{role}' has access to {len(tools)} tools from servers: {selected_servers}")
        return tools
    ...
    async def execute_batch(
        ...
        try:
            ...
            data = response.json()
            logger.info(f"Batch executed: {data.get('successful', 0)}/{data.get('total', 0)} successful")
