...
class MetaCoordinator:
    ...
    def _invoke_llm(self, messages: List, config: RunnableConfig):
        ...
        if "OpenAI" in llm_class_name:
            response = self.llm.invoke(messages, config=config, response_format={"type": "json_object"})
        else:
    ...
    def _create_fallback_plan(self, query: str) -> ExecutionPlan:
        ...
        if needs_web:
            agents = [
                {"role": "researcher", "task": "Search for current information", "can_delegate": False, "depends_on": []},
                {"role": "synthesizer", "task": "Create final answer", "can_delegate": False, "depends_on": [0]}
            ]
        else:
            agents = [
                {"role": "analyzer", "task": "Answer the question", "can_delegate": False, "depends_on": []}
            ]
